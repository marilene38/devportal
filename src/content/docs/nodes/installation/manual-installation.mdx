...
[repository](https://github.com/randlabs/algorand-windows-node) and
[accompanying tutorial](https://developer.algorand.org/tutorials/compile-and-run-the-algorand-node-natively-windows/).
...
> **Info**: The Key Manager Daemon (KMD) is Algorand's wallet process that manages keys and signing
...
These environment variables help you run `goal` commands without specifying the data directory each
time.
...
The service template above shows how `systemd` locates your `algod` binary. By default, it uses the
current working directory, but you can specify a different location using the `bindir` parameter,
