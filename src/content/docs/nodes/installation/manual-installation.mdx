...
3. Download and prepare the updater script:
...
5. Configure your environment by adding to your shell config file (`~/.bashrc` or `~/.zshrc`):
6. Configure your environment by adding to your shell config file (`~/.bashrc` or `~/.zshrc`):
...
3. Specify binary location (optional):
...
3. Use the catchup command with your network's current catchpoint from from the catchpoint URL:
