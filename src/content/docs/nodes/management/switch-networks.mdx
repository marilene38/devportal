...
This applies if the node is configured to run as a [.plist service on macOS](/nodes/installation/manual-installation#running-the-node-as-a-macos-service). If your macOS node is run purely via command line, follow the [Unmanaged steps](#unmanaged-command-line-only-install) instead.
