...
Goal is the command line utility used to interact with the Algorand node. It communicates with `algod` and the `kmd` process to do things like: create an account, list the ledger, examine the status of the network, or create a transaction. Goal documentation is available in the [Goal](FUTURELINK) guide.
...
See [algokey](FUTURELINK) documentation for more details.
