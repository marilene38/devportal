...
Warning: Do not use fast catchup on archival or relay nodes. If you accidentally do so, you must reset your node and start from scratch.
...
_*Warning*_: This process will remove all keys stored in the node. Proceed with caution.
...
> **Warning**: Running `diagcfg` as root will only affect nodes run as root, which is not recommended for security reasons.
...
> **Note**: Nodely's telemetry service is designed for MainNet nodes including validators, API nodes, and P2P nodes/relays. Do not use it for the Relay/Archiver Running Programme.
