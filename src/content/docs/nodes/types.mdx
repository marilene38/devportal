...
In addition to the two node types, nodes can be configured to be [**archival**](#archival-mode). Archival nodes store the entire ledger, as opposed to the last 1000 blocks for non-archival blocks. Relay nodes are necessarily archival. Non-relay archival nodes are often used to feed an [**indexer**](/nodes/indexer-installation) that allows more advanced queries on the history of the blockchain.
...
| non-relay | archival     | non-participation | ✅ normal configuration for an **archival node**, often connected to an [**indexer**](/nodes/indexer-installation)                                                              |
...
- I want full access to historical data (blocks, transactions) with advanced querying:
  - ➥ non-relay archival non-participation node, together with an [**indexer**](/nodes/indexer-installation).
- I want to get state proofs for any block:
  - ➥ non-relay archival non-participation node.

## Participation Node

How to install a node is described [here](/nodes/node-installation). Classifying a node as a participation node is not a configuration parameter but a dynamic operation where the node is hosting participation keys for one or more online accounts. This process is described in [Participate in Consensus](/nodes/consensus/overview). Technically both non-relay and relay nodes can participate in consensus, but Algorand recommends _only_ non-relay nodes participate in consensus.
