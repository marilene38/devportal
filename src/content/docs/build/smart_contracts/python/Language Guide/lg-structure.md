...
[logic signatures](), and [compile-time constant code and values](../../language-guide/lg-modules).
...
\\\\
[some inner transaction types](../../language-guide/lg-transactions#inner-transaction-objects-cannot-be-passed-to-or-returned-from-subroutines)
).
...
including reading directly from the transaction arguments via [`Txn`](../../api-reference/api-algopy.op#algopy.op.Txn). This makes
...
* `self.counter` will be stored in the application’s [Global State](../../language-guide/lg-storage#global-state).
...
See the [ARC-4 section](../../language-guide/lg-arc4) of this language guide for more info on the above.
