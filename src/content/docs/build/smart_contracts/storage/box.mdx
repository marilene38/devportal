...
import boxRefTestPython from '/examples/smart_contracts/py/storage-box-ref-test.py?raw';
import {
  readBoxDataTEAL,
  writeBoxDataTEAL,
  extractBoxDataTEAL,
...
- Applications that want to store data per user but do not wish to require users to [opt in](/build/accounts/overview) to the contract or need the account data to persist even after the user closes or clears out of the application.
...
When interacting with apps via [app call transactions](/build/transactions/overview), developers need a way to specify which boxes an application will access during execution. The box array is part of the [smart contract reference arrays](/build/smart_contracts/reference_arrays) alongside the apps, accounts, and assets arrays. These arrays define the objects the app call will interact with (read, write, or send transactions to).
