...
The `algokit project link` command is a powerful feature designed to streamline the integration between `frontend` and `contract` typed projects within the AlgoKit ecosystem. This command facilitates the automatic path resolution and invocation of [`algokit generate client`](/build/algokit/cli/features/generate#typed-clients) on `contract` projects available in the workspace, making it easier to integrate smart contracts with frontend applications.
...
```d2
direction: right
"Frontend Project" -> "Contract Project 1" -> "Frontend Project": "algokit generate cliet" {
    style: {
        fill: lightgray
    }
}
"Frontend Project" -> "Contract Project 2" -> "Frontend Project": "algokit generate cliet" {
    style: {
        fill: lightgray
    }
}
"Frontend Project" -> "Contract Project N" -> "Frontend Project": "algokit generate cliet" {
    style: {
        fill: lightgray
    }
}
"Frontend Project".style.fill: pink
```
