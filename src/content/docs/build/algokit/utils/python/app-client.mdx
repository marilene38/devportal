...
import createExample from '/src/examples/algokit/py/algokit-utils-py-app-client-create.py?raw';
import callAppMethodsExample from '/src/examples/algokit/py/algokit-utils-py-app-client-call-app-methods.py?raw';
import abiMethodsExample from '/src/examples/algokit/py/algokit-utils-py-app-client-abi-methods.py?raw';
import composingExample from '/src/examples/algokit/py/algokit-utils-py-app-client-composing.py?raw';
...
By generating an app client, you are provided with a high productivity application client that enables
...
[ARC-0032](https://arc.algorand.foundation/ARCs/arc-0032) application spec and
...
:::note
If you are using the [AlgoKit CLI](/build/algokit/getting-started) to bootstrap and manage your project, the included build scripts will generate your app client for you. The generated client will be placed in the `smart_contracts/artifacts/{app_name}` directory.
:::

## Using the Generated App Client
...
<Code
  code={composingExample}
  lang='py'
  frame='none'
  mark='AtomicTransactionComposer'
/>
...
<Aside type='caution' title='Enabling debug mode'>
  Remember to enable debug mode (`config.debug = True`) to include raw simulate
  execution traces in the `LogicError`.
</Aside>
