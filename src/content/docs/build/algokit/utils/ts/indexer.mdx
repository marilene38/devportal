...
title: Indexer
prev: 
    link: /build/algokit/utils/ts/amount/
next:
...
import pagination from '/examples/algokit/utils-ts/algokit-utils-ts-indexer.ts?raw';

Indexer lookups / searching is a higher-order use case capability provided by AlgoKit Utils that builds on top of the core capabilities.
It provides type-safe indexer API wrappers - no more `Record<string, any>` pain including automatic pagination control.
...
- [`algokit.lookupTransactionById(transactionId, indexer)`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/code/modules/index.md#lookuptransactionbyid) - Finds a transaction by ID
- [`algokit.lookupAccountByAddress(accountAddress, indexer)`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/code/modules/index.md#lookupaccountbyaddress) - Finds an account by address
- [`algokit.lookupAccountCreatedApplicationByAddress(indexer, address, getAll?, paginationLimit?)`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/code/modules/index.md#lookupaccountcreatedapplicationbyaddress) - Finds all applications created for an account
- [`algokit.lookupAssetHoldings(indexer, assetId, options?, paginationLimit?)`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/code/modules/index.md#lookupassetholdings) - Finds all asset holdings for the given asset
- [`algokit.searchTransactions(indexer, searchCriteria, paginationLimit?)`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/code/modules/index.md#searchtransactions) - Search for transactions with a given set of criteria
- [`algokit.executePaginatedRequest(extractItems, buildRequest)`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/code/modules/index.md#executepaginatedrequest) - Execute the given indexer request with automatic pagination
...
```typescript
const transactions = await algokit.searchTransactions(indexer, s =>
