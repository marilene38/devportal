...
import kmdAccount from '/src/examples/algokit/utils-ts/algokit-utils-ts-account-manager.ts?raw';
...
To get an instance of the `KmdAccountManager` class you can access it from [`AlgorandClient`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/capabilities/algorand-client.md) via `algorand.account.kmd` or instantiate it directly (passing in a [`ClientManager`](/build/algokit/utils/ts/client)):
