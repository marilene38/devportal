...
A single signature corresponds to a signature from the private key of an [Algorand public/private key pair](/FUTURE_LINK).
...
When the [sender](/concepts/transactions/transactions#sender) of a transaction is the address of a [multisignature account](./FUTURE_LINK) then authorization requires a subset of signatures, _equal to or greater than the threshold value_, from the associated private keys of the addresses that multisignature account is composed of. See [Multisignature Accounts](./FUTURE_LINK) for details on how to configure a multisignature account.
