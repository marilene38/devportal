...
Depending on your requirement, if you need an account for sending and re [Standalone Accounts](#standalone), [KMD-Managed Accounts](#kmd-managed-accounts) using [kmd](/nodes/reference/artifacts#kmd), or [Multi-signature accounts](/concepts/accounts/multisig) which entails one of the prior methods.

This section will explore standalone accounts and wallet-derived accounts in-depth, guiding how to utilize them in `algokit-utils` , `goal`, `algokey`, `SDKs`, and `Pera Wallet`, the reasons you might want to choose one method over another for your application.
