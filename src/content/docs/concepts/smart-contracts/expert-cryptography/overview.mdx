---
title: Overview
description: Overview of the Expert Cryptography section, for producing applications utilizing advanced cryptography features in the AVM.
---

## Introduction

The Algorand Virtual Machine (AVM) contains opcodes allowing for applications utilizing more advanced cryptography. This section aims to elucidate for the more experienced smart contract developers and applied cryptography afficionados how to make use of those opcodes.
...
Opcodes flagged with "vFuture" have been implemented in the node softsware in draft, but are have not been pushed out to production.
...
Note that SHA512/256 is not the same as `SHA512 % 2^256`.
...
Note that the BN254 curve is also known under `alt_bn128` and is *NOT* to be confused with `Fp254BNb`.
