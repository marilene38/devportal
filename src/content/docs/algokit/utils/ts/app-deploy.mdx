...
- If `create`, `update` or `replace` then it will have [`ConfirmedTransactionResults`](/build/algokit/utils/ts/transaction#send-transaction-result) and `{return?: ABIReturn}` (which will be populated if an ABI method was executed)
