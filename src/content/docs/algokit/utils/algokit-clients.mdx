...
The easiest way to interact with the Algorand blockchain and also deployed smart contracts is to use the AlgoKit Utils clients.
These clients have been designed to expose intuitive methods that are grouped conceptually for ease of exploration through your development environment.
...
Typed application clients are automatically generated, typed TypeScript or Python deployment and interaction clients for smart contracts that have a defined [ARC-56](/standards/arcs/arc-0056) or [ARC-32](/standards/arcs/arc-0032) application specification so that the development experience is easier with less upskill ramp-up and less deployment errors. These clients give you a type-safe, intellisense-driven experience for calling the smart contract.
...
Use it to craft and send transactions individually or bundled as atomic transaction groups, access REST API clients (`algod`, `indexer`, `kmd`), and quickly set configuration parameters, with sensible defaults.
...
### Accessing the REST API clients

Once you have an `AlgorandClient` instance, you can access the REST API clients for the various Algorand APIs via the `algorand.client` property:
...
Typed application clients are automatically generated, typed TypeScript or Python deployment and interaction clients for smart contracts that have a defined `ARC-56` or `ARC-32` application specification so that the development experience is easier with less upskill ramp-up and less deployment errors. These clients give you a type-safe, Intellisense-driven experience for calling the smart contract.
